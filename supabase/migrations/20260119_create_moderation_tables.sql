...
-- ============================================
-- Purpose: Track violations, user reports, and implement progressive penalties
-- Created: 2026-01-19

-- ============================================
-- 1. Moderation Logs Table
-- ============================================
-- Records all content violations (NSFW images, sensitive prompts)
...
-- ============================================
-- User-generated reports for inappropriate content in gallery
...
-- Index for generation lookups (prevent duplicate reports)
CREATE INDEX idx_reports_generation ON user_reports(generation_id);

-- Index for reporter lookups (prevent spam reporting)
...
-- 3. User Bans Table
-- ============================================
-- Track temporary and permanent bans
...
CREATE TABLE IF NOT EXISTS user_bans (
  ...
  created_at timestamp with time zone DEFAULT now(),
  created_by uuid REFERENCES auth.users(id) ON DELETE SET NULL -- Admin who issued ban
);

-- Index for checking active bans
-- Note: Removed WHERE clause with now() as it's not IMMUTABLE
-- Query will filter at runtime instead
...
-- Function: Log a violation (called from API)
...
-- Moderation Logs: Service role can insert (from API)
...
-- User Reports: Admins can view all reports
...
-- User Reports: Admins can update reports
...
-- User Bans: Users can view their own ban status
CREATE POLICY "Users can view their own ban status"
  ON user_bans
  FOR SELECT
  USING (user_id = auth.uid());

-- User Bans: Admins can view all bans
...
-- User Bans: Admins can create/update bans
...
BEGIN
  ...
  RAISE NOTICE '- moderation_logs: Violation tracking';
  RAISE NOTICE '- user_reports: User-submitted reports';
  RAISE NOTICE '- user_bans: Ban management';
  RAISE NOTICE '- Helper functions: get_user_violation_count, is_user_banned, log_violation';
END $$;
